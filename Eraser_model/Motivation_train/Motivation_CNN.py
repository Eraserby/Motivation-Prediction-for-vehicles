...
# 搭建卷积神经网络
class CNN(nn.Module):
    def __init__(self , in_channel=5 ,feature=8 ,num_class=3) :
        super(CNN,self).__init__()
        self.Conv1 = nn.Conv2d(in_channel,feature,kernel_size=2,stride=1,padding=0)
        # batch_size x 1 x 150 x 2 --> batch_size x feature x 149 x 1
        self.Relu1 = nn.ReLU()
        self.Fc1 = nn.Linear(feature*149*1 , num_class)
    ...
    def forward(self , x):
        x=self.Conv1(x)
        x=self.Relu1(x)
        # x=x.reshape(x.shape[0] , -1) # batch_size ,feature, 149 ,1 -->
                                        # batch_size,feature x 149 x 1
        x=self.Fc1(x)
        return x
...
epochs = 20
# 初始化函数
model = CNN(in_channel=5 , feature=8,num_class=3).to(dev)
...
for epoch in range(epochs):
    ...
    model_dir = "saved_model/Motivation_CNN_固定frame"
    os.makedirs(model_dir, exist_ok=True)
    torch.save(model.state_dict(), model_dir + "/CNN_epoch{}".format(epoch+21))
