...
for epoch in range(epochs):
    ...
    for batch_index , (inputs, labels) in enumerate(tqdm(train_loader)):
        # 把维度进行变换
        inputs = inputs.reshape(-1,num_input).to(dev) # batch_size x150x2 ---> batch_size x 200
    ...
    model_dir = "saved_model/Motivation_MLP_150frame_x_y"
...
plt.title('single MLP Training Loss vs. Epoch frame=150')
