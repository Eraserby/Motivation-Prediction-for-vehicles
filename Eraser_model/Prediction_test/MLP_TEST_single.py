...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pred_traj_gt = agent_traj[self.obs_len : self.obs_len + 50] 
        # # 一步实现标准化
        # scaler = StandardScaler()#实例化
        # # 使用 fit 方法拟合数据
        # scaler.fit(obs_traj)
        # scaler.fit(pred_traj_gt)
        # obs_traj = scaler.transform(obs_traj)#一步达成结果
        # pred_traj_gt = scaler.transform(pred_traj_gt)
...
loaded_params = torch.load(
    'D:\\Eraser_python\\pre_trajectory_for_vehicles\\saved_model\\MLP_2\\MLP_epoch99')  
...
if __name__ == "__main__":
    with torch.no_grad():  # 确保在这个上下文中不计算梯度，节省内存并加快速度
        ...
        # 可视化
        # scaler = StandardScaler()#实例化
        # predictions = scaler.inverse_transform(predictions)#逆转标准化
        # ground_truths = scaler.inverse_transform(ground_truths)
        # history_truths = scaler.inverse_transform(history_truths)
