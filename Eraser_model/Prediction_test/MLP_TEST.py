...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return [agent_traj[:self.obs_len], agent_traj[self.obs_len+1:self.obs_len+26]]
...
class MLP(nn.Module):
    """
    期望输入为(batch_size, 25,2)
    25:输入序列长度
    2:输入特征的维度(x和y)
    输出形状:(batch_size, 25 * 2)
    ...
    def forward(self, x):
        # 将 (batch_size, 25, 2) 转换为 (batch_size, 25 * 2)
        x = x.view(x.size(0), -1)
...
loaded_params = torch.load(
    'D:\\Eraser_python\\pre_trajectory_for_vehicles\\saved_model\\MLP_1\\MLP_epoch0')  
