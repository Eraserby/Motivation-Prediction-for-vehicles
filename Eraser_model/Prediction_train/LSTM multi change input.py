...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        random_indices = torch.randperm(len(array_elements))[0]
        # # 使用随机索引获取随机元素
        # andom_element = array_elements[random_index]

        # for num in random_index: # 使用列表进行遍历
        # for random_index in random_indices:
        random_element = array_elements[random_indices]
        x_vehID = sequence[sequence['id']== random_element]
        # 第id辆车的信息
        x_vehID = x_vehID.sort_values(by='frame')
        agent_x = x_vehID['x']
        agent_y = x_vehID['y']
        ...
        # agent_traj = scaler.fit_transform(agent_traj) # 一步达成标准化
...
class LSTM(nn.Module):
    ...
    def forward(self, input_seq):
        # 初始化隐藏状态和记忆单元
        h_0 = torch.zeros(self.num_layers, input_seq.size(0), self.hidden_size)
        c_0 = torch.zeros(self.num_layers, input_seq.size(0), self.hidden_size)
        hidden = (h_0, c_0)

        lstm_out, self.hidden = self.lstm(input_seq, hidden) 
...
for epoch in range(epoches):
    ...
    epoch_loss = 0.0
    RMSE_epoch_loss = 0.0
    for i ,xb  in enumerate(tqdm(train_loader)):
